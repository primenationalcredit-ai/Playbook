import sys, re
# ---- 1) the PDF generator function (zero dependencies) ----
fn = r"""// netlify/functions/sop-pdf-background.js
//
...
"""
open('netlify/functions/sop-pdf-background.js', 'w', encoding='utf-8', newline='').write(fn)
print("sop-pdf-background.js written")
# ---- 2) SopLibrary: fire the sweep on load + PDF button per row ----
f = 'src/pages/SopLibrary.jsx'
s = open(f, encoding='utf-8', errors='surrogateescape').read()
a1 = "const cards = await supabaseFetch('project_cards'"
if s.count(a1) != 1: print("ABORTED: load anchor x" + str(s.count(a1))); sys.exit(1)
s = s.replace(a1, "fetch('/.netlify/functions/sop-pdf-background', { method: 'POST' }).catch(() => {}); // PDFs for any SOP missing one (Joe 8/13)\n      " + a1, 1)
a2 = '<div className="text-xs text-slate-400 shrink-0">{s.label}</div>'
if s.count(a2) != 1: print("ABORTED: label anchor x" + str(s.count(a2))); sys.exit(1)
badge = a2 + """
              {s.pdf_url && (
...
if not re.search(r"import \{[^}]*\bDownload\b[^}]*\} from 'lucide-react'", s):
    lm = re.search(r"import \{([^}]*)\} from 'lucide-react'", s)
    if not lm: print("ABORTED: lucide import not found"); sys.exit(1)
    s = s.replace(lm.group(0), "import {" + lm.group(1) + ", Download } from 'lucide-react'", 1)
open(f, 'w', encoding='utf-8', errors='surrogateescape', newline='').write(s)
print("SopLibrary wired: sweep on load + PDF button")
