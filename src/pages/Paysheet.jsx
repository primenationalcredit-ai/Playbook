...
function Paysheet() {
  ...
  // Fetch sales data - LIVE from Google Sheets (single call for multiple months)
  const fetchSalesMultiMonth = async (monthsList) => {
    ...
    (data || []).forEach(row => {
      ...
      if (!map[name]) {
        map[name] = {
          ...
          bonusCommission: 0,
          affiliateDeductTotal: 0,
          count: 0
        };
      }
      
      const amount = parseFloat(row.fee_paid) || 0;
      const affiliateDeduct = parseFloat(row.affiliate_deduct) || 0;
      ...
      // Track affiliate deductions: when P column has a value,
      // the deduction = (D - P) because commission is on P instead of D
      if (affiliateDeduct > 0) {
        map[name].affiliateDeductTotal += (amount - affiliateDeduct);
      }
      
      // Doc Fee Race: count based on CODE column = "Doc Fee" (not fee_type)
      if (code.includes('doc')) {
        map[name].docs++;
        const period = getPeriod(row.date_paid);
        map[name][period]++;
        
        // Same Day Doc: column M (Doc Paid Date) is populated for same-day docs
  ...
  const calculatePay = (salesName, userName, consultantsData, winners, lastMonthRefunds) => {
    ...
    // Affiliate deductions (only for non-VA US consultants)
    // When column P has a value, commission is on P instead of D
    // Deduction = sum of (D - P) * commissionRate for those rows
    const affiliateDeduction = isVA ? 0 : consultant.affiliateDeductTotal * commissionRate;
    
    const grandTotal = baseCommission + totalPotentialBonuses - chargebacks - affiliateDeduction;
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const lastMonth = format(startOfMonth(subMonths(selectedMonth, 1)), 'yyyy-MM');

      // SINGLE API call for both months (reads Google Sheet once)
      const monthsData = await fetchSalesMultiMonth([currentMonth, lastMonth]);
      const currentData = monthsData[currentMonth]?.rows || [];
      const lastMonthData = monthsData[lastMonth]?.rows || [];
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white rounded-lg p-4 shadow-sm">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {!payData.isVA && payData.affiliateDeduction > 0 && (
                    <div className="flex justify-between">
                      <span className="text-slate-500">Affiliate Deduct</span>
                      <span className="font-semibold text-red-600">-{formatCurrency(payData.affiliateDeduction)}</span>
                    </div>
                  )}
                </div>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="text-center px-3 py-2">Commission</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {allConsultantsData.map((c, idx) => (
                      <tr key={c.name} className={c.name === selectedConsultant ? 'bg-emerald-50' : ''}>
                        ...
                        <td className="text-center px-3 py-2">{c.docs || '-'}</td>
                        <td className="text-center px-3 py-2">{formatCurrency(c.commission)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
