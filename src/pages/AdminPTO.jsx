...
const ACCRUAL_TYPES = [
  { id: 'semi_monthly', name: 'Semi-Monthly (1st & 15th)', description: 'Accrues 4 hrs each payroll on the 1st and 15th' },
  { id: 'monthly', name: 'Monthly', description: 'Accrues once per month' },
  { id: 'per_pay_period', name: 'Per Pay Period', description: 'Accrues each pay period' },
];
...
export default function AdminPTO() {
  ...
  const savePolicy = async (policyData) => {
    try {
      let res;
      if (editingPolicy) {
        res = await fetch(`${SUPABASE_URL}/rest/v1/pto_policies?id=eq.${editingPolicy.id}`, {
          method: 'PATCH',
          headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
          body: JSON.stringify({ ...policyData, updated_at: new Date().toISOString() })
        });
      } else {
        res = await fetch(`${SUPABASE_URL}/rest/v1/pto_policies`, {
          method: 'POST',
          headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
      ...
      if (!res.ok) {
        ...
        console.error('Save policy failed:', res.status, errText);
        alert(`Failed to save policy: ${errText}`);
        return;
      }
      await loadPolicies();
  ...
  const adjustBalance = async (userId, amount, description) => {
    try {
      ...
      let res;
      if (currentBalance) {
        res = await fetch(`${SUPABASE_URL}/rest/v1/pto_balances?user_id=eq.${userId}`, {
          method: 'PATCH',
          headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
          body: JSON.stringify({ balance: newBalance, updated_at: new Date().toISOString() })
        });
      } else {
        res = await fetch(`${SUPABASE_URL}/rest/v1/pto_balances`, {
          method: 'POST',
          headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
          body: JSON.stringify({ user_id: userId, balance: newBalance, used: 0, pending: 0 })
        });
      }
      if (!res.ok) {
        const errText = await res.text();
        console.error('Adjust balance failed:', res.status, errText);
        alert(`Failed to adjust balance: ${errText}`);
        return;
      }

      // Record transaction
      const txRes = await fetch(`${SUPABASE_URL}/rest/v1/pto_transactions`, {
        method: 'POST',
        headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
        body: JSON.stringify({
          user_id: userId,
      ...
      if (!txRes.ok) console.error('Transaction log failed:', await txRes.text());

      await loadData();
      setShowAdjustModal(false);
      setSelectedUser(null);
      alert(`PTO balance updated: ${amount > 0 ? '+' : ''}${amount} hrs. New balance: ${newBalance} hrs.`);
    } catch (error) {
    ...
    try {
      ...
      await fetch(`${SUPABASE_URL}/rest/v1/pto_transactions`, {
        ...
        body: JSON.stringify({
          ...
          description: `Assigned to ${policy?.name} policy with ${startingBalance} hrs`,
          created_by: currentUser?.id
        })
      });
                    ...
                    <tr className="border-t border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="p-4 text-center">
                        <span className={`text-xl font-bold ${
                          (balance?.balance || 0) <= 16 ? 'text-red-600' : 
                          (balance?.balance || 0) <= 40 ? 'text-amber-600' : 'text-green-600'
                        }`}>
                      ...
                      <td className="p-4 text-center text-slate-600">
                        {balance?.used?.toFixed(1) || '0.0'} hrs
                      </td>
                      <td className="p-4 text-center">
                        {(balance?.pending || 0) > 0 ? (
                          <span className="text-amber-600">{balance.pending.toFixed(1)} hrs</span>
                        ) : (
                                ...
                                <div key={txn.id} className="flex items-center justify-between bg-white p-3 rounded-lg">
                                  ...
                                  <div className="text-right">
                                    <p className={`font-semibold ${txn.amount > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                      {txn.amount > 0 ? '+' : ''}{txn.amount.toFixed(1)} hrs
                                    </p>
            ...
            <div key={policy.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-slate-600">Hrs/Payroll</span>
                  <span className="font-semibold text-green-600">{policy.annual_days} hrs</span>
                </div>
                <div className="flex justify-between">
                ...
                {policy.allow_rollover && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-800">{policy.max_rollover_days} hrs</span>
                  </div>
                )}
                {policy.max_balance && (
                  <div className="flex justify-between">
                    <span className="text-slate-600">Max Balance</span>
                    <span className="text-slate-800">{policy.max_balance} hrs</span>
                  </div>
                )}
              </div>
...
function PolicyModal({ policy, onClose, onSave }) {
  ...
  const [hoursPerPayroll, setHoursPerPayroll] = useState(policy?.annual_days || 4);
  const [accrualType, setAccrualType] = useState(policy?.accrual_type || 'semi_monthly');
  ...
  const [maxRollover, setMaxRollover] = useState(policy?.max_rollover_days || 40);
  ...
  const handleSubmit = (e) => {
    ...
    onSave({
      name,
      description,
      annual_days: parseFloat(hoursPerPayroll),
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl my-8">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Hours Per Payroll *</label>
            <input type="number" step="0.5" value={hoursPerPayroll} onChange={(e) => setHoursPerPayroll(e.target.value)} className="w-full px-4 py-2 border rounded-lg" required />
          </div>
          <div>
          ...
          <div className="border-t pt-4">
            ...
            {allowRollover && (
              <div className="ml-6">
                <label className="block text-sm text-slate-600 mb-1">Maximum Rollover Hours</label>
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="text-center p-4 bg-slate-50 rounded-xl">
            ...
            <p className="text-3xl font-bold text-slate-800 mt-1">{currentBalance.toFixed(1)} hrs</p>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <select value={policyId} onChange={(e) => setPolicyId(e.target.value)} className="w-full px-4 py-2 border rounded-lg" required>
              ...
              {policies.map(p => <option key={p.id} value={p.id}>{p.name} ({p.annual_days} hrs/payroll)</option>)}
            </select>
          </div>
            ...
            <>
              <div className="p-4 bg-green-50 rounded-xl space-y-2">
                ...
                <div className="text-sm text-green-600">
                  <p>• {selectedPolicy.annual_days} hrs per payroll (1st & 15th)</p>
                  <p>• {selectedPolicy.expires ? `Expires: ${selectedPolicy.expiration_type}` : 'Never expires'}</p>
                  {selectedPolicy.allow_rollover && <p>• Rollover up to {selectedPolicy.max_rollover_days} hrs</p>}
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Starting Balance (hours)</label>
