...
function Dashboard() {
  ...
  const loadUpcomingPTO = async () => {
    try {
      const today = new Date().toISOString().split('T')[0];
      const url = `${SUPABASE_URL}/rest/v1/time_off_requests?user_id=eq.${currentUser.id}&status=eq.approved&end_date=gte.${today}&select=*&order=start_date.asc&limit=3`;
      const res = await fetch(url, {
        headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}` }
      });
      if (res.ok) {
        setUpcomingPTO(await res.json() || []);
      }
    } catch (err) {
      console.log('Failed to load upcoming PTO:', err);
    }
  };
  ...
  // Valid time slot IDs
  const validTimeSlots = Object.values(TIME_SLOTS).map(s => s.id);
  
  // Filter to only tasks with valid time slots
  const validTasks = tasks.filter(t => validTimeSlots.includes(t.timeSlot));
  
  const completedTasks = validTasks.filter(t => t.completed);
  const overdueTasks = validTasks.filter(t => !t.completed && t.specificTime && isOverdue(t.specificTime));
  const upcomingTasks = validTasks.filter(t => !t.completed && !isOverdue(t.specificTime));
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard
          icon={TrendingUp}
          label="Progress"
          value={`${validTasks.length > 0 ? Math.round((completedTasks.length / validTasks.length) * 100) : 0}%`}
          color="amber"
        />
      </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
            ...
            <div className="mb-6">
              <div className="flex justify-between text-sm mb-2">
                <span className="text-slate-600">{completedTasks.length} of {validTasks.length} tasks completed</span>
                <span className="font-semibold text-asap-gold">{validTasks.length > 0 ? Math.round((completedTasks.length / validTasks.length) * 100) : 0}%</span>
              </div>
              <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ width: `${validTasks.length > 0 ? Math.round((completedTasks.length / validTasks.length) * 100) : 0}%` }}
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {Object.values(TIME_SLOTS).map(slot => {
                const slotTasks = validTasks.filter(t => t.timeSlot === slot.id);
        ...
        <div className="space-y-6">
          ...
          {ptoBalance && (() => {
            ...
            <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-6 text-white">
              ...
              <p className="text-4xl font-bold mb-1">{available.toFixed(1)}</p>
              <p className="text-green-100 text-sm mb-4">hrs available</p>
              <div className="flex justify-between text-sm">
                <div>
                  <p className="text-green-200">Used</p>
                  <p className="font-semibold">{ptoBalance.used?.toFixed(1) || '0'} hrs</p>
                </div>
                <div className="text-right">
                  <p className="text-green-200">Upcoming</p>
                  <p className="font-semibold">{ptoBalance.pending?.toFixed(1) || '0'} hrs</p>
                </div>
              </div>
              {upcomingPTO.length > 0 && (
                <div className="mt-3 pt-3 border-t border-white/20">
                  <p className="text-green-100 text-xs mb-1">Next Time Off</p>
                  {upcomingPTO.map(req => (
                    <div key={req.id} className="text-sm flex justify-between mt-1">
                      <span>{req.start_date} — {req.end_date}</span>
                      <span className="bg-white/20 px-2 py-0.5 rounded text-xs">
                        {req.request_type === 'vacation' ? '🏖️' : req.request_type === 'personal' ? '👤' : '🤒'} {req.request_type}
                      </span>
                    </div>
                  ))}
                </div>
