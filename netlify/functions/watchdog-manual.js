...
const runWatchdog = async (event) => {
  ...
  try {
    ...
    // 5.4) PARTIAL coverage (Joe 7/31): the watchdog was finals-only - a
    ...
    if ((report.healed.length || report.partials_healed.length) && !dryRun) {
      fetch(`https://cute-cat-d9631c.netlify.app/.netlify/functions/consultant-bonus-metrics?month=${month}&refresh=1`).catch(() => {});
    }
    // 5.7) DUPLICATE-PAYMENT SCAN (Erik class): same client + amount + type +
