...
//
// THE review check (moved here 9/19). review-reconcile.js is now only its daily
// schedule, which calls this over HTTP. Netlify rejects direct calls to scheduled
// functions and require() between function files returns nothing in this repo,
// so the logic lives here where it can be both scheduled and run on demand.
// Keyed: pass X-API-Key.
//
// review-reconcile.js
...
exports.handler = async (event) => {
  ...
  const _k = (event.headers && (event.headers['x-api-key'] || event.headers['X-API-Key'])) || ((event.queryStringParameters || {}).key);
  if (!process.env.INTERNAL_API_KEY || _k !== process.env.INTERNAL_API_KEY) return { statusCode: 401, headers, body: JSON.stringify({ error: 'Invalid API key' }) };
  try {
