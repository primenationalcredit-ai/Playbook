...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (params.test === '1' || params.test === 'true') {
      ...
      let emailResult = null, smsResult = null, surveyLink = null;
      try {
        const sres = await fetch(SENDER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(t) });
        const sjson = await sres.json().catch(() => ({}));
        emailResult = sjson.email || null; smsResult = sjson.sms || null; surveyLink = sjson.link || null;
    ...
    for (const t of toSend.slice(0, cap)) {
      ...
      const _n = await notifyAfterSend(t, emailResult, smsResult, surveyLink);
