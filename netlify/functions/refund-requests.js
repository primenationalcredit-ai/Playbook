...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (b.action === 'submit') {
      ...
      // Consultant = Pipedrive deal owner at request time (display + payroll attribution)
      let consultantName = null;
      try {
        const dr = await fetch(`https://${PIPEDRIVE_DOMAIN}.pipedrive.com/api/v1/deals/${encodeURIComponent(String(b.pipedrive_deal_id))}?api_token=${PIPEDRIVE_TOKEN}`);
        const dd = await dr.json();
        consultantName = (dd && dd.data && (dd.data.owner_name || (dd.data.user_id && dd.data.user_id.name))) || null;
      } catch (e) {}
    ...
    if (b.action === 'release_signed') {
      ...
      const rows = await supa(`refund_requests?id=eq.${b.request_id}&select=id,status,amount,pipedrive_deal_id,client_name,client_email,consultant_name,reason,deduction_recorded`);
      const req = (rows.json || [])[0];
      if (!req) return respond(404, { error: 'Request not found' });
      const upd = await supa(`refund_requests?id=eq.${b.request_id}`, {
        method: 'PATCH', headers: { Prefer: 'return=minimal' },
    ...
    if (b.action === 'check_mailed') {
      ...
      const rows = await supa(`refund_requests?id=eq.${b.request_id}&select=id,status,pipedrive_deal_id,client_name,client_email,check_amount,card_refunded_amount,amount,consultant_name,reason,deduction_recorded`);
      const req = (rows.json || [])[0];
      if (!req) return respond(404, { error: 'Request not found' });
      ...
      // Bonus/paysheet automation (Joe 8/4): a mailed check IS the refund.
