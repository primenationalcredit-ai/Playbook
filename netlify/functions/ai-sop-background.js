...
exports.handler = async (event) => {
  ...
  try {
    ...
    const card = rows[0];
    const tasks = (card.steps || []).map((st, i) => (i + 1) + '. [' + (st.done ? 'BUILT' : 'NOT BUILT YET') + '] ' + st.text + (st.assignee ? ' (owner: ' + st.assignee + ')' : '') + (st.details ? '\n   Details: ' + String(st.details).slice(0, 800) : '') + (Array.isArray(st.subtasks) && st.subtasks.length ? '\n   Checklist: ' + st.subtasks.map(sb => sb.text).join(' | ').slice(0, 1200) : '')).join('\n');
    const allSteps = Array.isArray(card.steps) ? card.steps : [];
