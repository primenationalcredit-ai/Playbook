...
// Background function (15-min budget) that scans the full CRS pipeline once and caches
// each deal's round dates, so credit-team-bonus-metrics can compute cohort + 4th-round
// instantly from the cache instead of crawling 12k+ deals on a page load.
// Writes app_cache['credit_team_round_dates']. Trigger via cron or lazily from the metrics fn.
  ...
  try {
    ...
    while (more && pages < 300) {
      ...
      for (const d of (r.data || [])) {
        ...
        if (a || e || dd) out.push({ a, c, e, d: dd }); // keep only clients with relevant round data
      }
