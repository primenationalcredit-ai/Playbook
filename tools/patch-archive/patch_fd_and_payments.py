...
s = open(f, encoding='utf-8').read()
old = "consultant_payments?payment_month=eq.${ym}&excluded_from_bonus=eq.false&select="
new = "consultant_payments?payment_month=eq.${ym}&excluded_from_bonus=eq.false&refunded_at=is.null&select="
if s.count(old) != 1: print(f"ABORTED: payments-live anchor x{s.count(old)}"); sys.exit(1)
s = s.replace(old, new, 1)
open(f, 'w', encoding='utf-8', newline='').write(s)
print("payments-live: refunded payments never reach the payments page")
