import sys, os
D = """import React, { useMemo } from 'react';
...
"""
open('src/components/SopDocument.jsx','w',encoding='utf-8',newline='').write(D)
print("1/2 SopDocument.jsx written")

f='src/pages/LeadershipProjects.jsx'; s=open(f,encoding='utf-8',errors='surrogateescape').read()
if 'SopDocument' in s: print("SKIP 2/2"); sys.exit(0)
a="import { supabase } from '../lib/supabase';"
if s.count(a)!=1:
    import re
    m=re.search(r"^import .*\n", s, re.M); a=m.group(0).rstrip('\n')
    if s.count(a)!=1: print("ABORT import anchor"); sys.exit(1)
s=s.replace(a, a+"\nimport SopDocument from '../components/SopDocument';",1)
...
if s.count(old)!=1: print("ABORT sop renderer x"+str(s.count(old))); sys.exit(1)
s=s.replace(old,'''                  {l.sop ? (
...
a2="            <div className=\"space-y-1.5 max-h-64 overflow-y-auto\">\n              {links.length === 0 &&"
if s.count(a2)!=1: print("ABORT links block x"+str(s.count(a2))); sys.exit(1)
s=s.replace(a2, "            {openSop && <SopDocument sop={openSop} projectTitle={card.title} onClose={() => setOpenSop(null)} />}\n"+a2,1)

a3="  const [newLink, setNewLink] = useState"
if s.count(a3)!=1: print("ABORT state anchor x"+str(s.count(a3))); sys.exit(1)
s=s.replace(a3, "  const [openSop, setOpenSop] = useState(null); // SOP document viewer (Joe 8/13)\n"+a3,1)
open(f,'w',encoding='utf-8',errors='surrogateescape',newline='').write(s)
print("2/2 LeadershipProjects.jsx: SOPs open in the document viewer")
